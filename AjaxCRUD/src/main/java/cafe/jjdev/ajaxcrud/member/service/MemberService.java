...
<<<<<<< HEAD
		...
		// 멤버 목록 조회 결과 변수에 담는다
		List<Member> list = memberMapper.selectMemberList(BEGIN_ROW, ROW_PER_PAGE);
		...
		/* 4. 하단 넘버링 시작할 번호 int */
		int startPageNo = ((currentPage - 1) / 10) * 10 + 1; // 숫자 페이징 시작번호 : 1, 11, 21, ...
		// 리턴할 객체에 시작번호 셋팅
		map.put("startPageNo", startPageNo);
		
		/* 5. 하단 넘버링 끝낼 번호 int */
		int endPageNo = startPageNo + 9; // 숫자 페이징 끝번호 : 10, 20, 30, ...
		if(endPageNo > lastPage) { // 숫자 페이징 끝번호가 아무리 커져도 마지막 페이지보다 커지면 안되므로
			endPageNo = lastPage;
		}
		// 리턴할 객체에 끝번호 셋팅
		...
		System.out.println("[MemberService] map (after setting) : " + map);
